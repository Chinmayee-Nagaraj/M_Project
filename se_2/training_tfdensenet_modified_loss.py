...
def train_tfdensenet():
    ...
    loss_fn =  CompositeSpeechEnhancementLoss(device).to(device)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for clean_wave, noisy_wave, _ in pbar:
            ...
            pbar.set_postfix({
                "Total": float(loss),
                "L1": float(loss_dict["L1_mag_loss"]),
                "LogMag": float(loss_dict["LogMag_loss"]),
                "SC": float(loss_dict["SpecConv_loss"]),
                "MR-STFT": float(loss_dict["MRSTFT_loss"]),
                "SISDR": float(loss_dict["SISDR_loss"]),
                "Mel": float(loss_dict["Mel_loss"]),
                })
