        ...
        for batch in pbar:  # batch is a list of samples
            ...
            for clean_wave, noisy_wave, length, fname in batch:     
                ...
                loss = (alpha_wave * loss_wave +
                        ...
                        alpha_sisdr * loss_sisdr +
                        alpha_stft * loss_stft)
...
def train_tfdensenet():
    ...
    # Dataloading
    train_loader, val_loader, test_loader = load_datasets(root_dir=dataset_dir, batch_size=4) #TODO
    ...
    if checkpoint_files:
        ...
        checkpoint_model = torch.load(ckpt_path, map_location=device, weights_only=True)
        model.load_state_dict(checkpoint_model["model_state_dict"])
    
        # 2. Load optimizer and scheduler states from checkpoint dictionary manually
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            checkpoint_full = torch.load(ckpt_path, map_location=device)
        
        optimizer.load_state_dict(checkpoint_full["optimizer_state_dict"])
        scheduler.load_state_dict(checkpoint_full.get("scheduler_state_dict", {}))
        start_epoch = checkpoint_full["epoch"]
        last_loss = checkpoint_full.get("loss", None)
        print(f"Resumed from checkpoint: {ckpt_path} (epoch {start_epoch})")
    else:
        print("Starting training from epoch 0")

    print("Loss weights: alpha_wave = {} | alpha_spect = {} | alpha_sisdr = {} | alpha_stft = {}".format(alpha_wave, alpha_spect, alpha_sisdr, alpha_stft))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        print(f"  Val   - Total: {val_loss:.4f}| Wave: {val_loss_wav:.4f}| Spect: {val_loss_spect:.4f}| -SiSDR: {val_loss_sisdr:.4f}| STFT: {avg_loss_stft:.4f}")
